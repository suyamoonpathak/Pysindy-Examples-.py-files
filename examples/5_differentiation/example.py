...
Original file is located at
    https://colab.research.google.com/drive/1elrbYy9_6i0iKehXMyj98O_a9Ros3eZw
...
diffs.append(("Smooth FD, reuse old x", ps.SmoothedFiniteDifference()))
diffs.append(
    (
        "Kalman, reuse old x",
        ps.SINDyDerivative(kind="kalman", alpha=0.5),
    )
)
...
ax = plot_sho(x_train, x_train_noisy)
ax.set_title("Savitzky-Golay filtered for Smoothed FD method")
plt.subplot(1, 2, 2)
# ax = plot_sho(x_train, x_train_noisy, diffs[8][1].smoothed_x_) // removed the last diffs part for both the lines
ax = plot_sho(x_train, x_train_noisy)
...
diffs[10] = (
    "Kalman, reuse old x",
    ps.SINDyDerivative(kind="kalman", alpha=0.0015),
)
